...
class SimpleRLPlayer(Gen8EnvSinglePlayer):
    ...
    def embed_battle(self, battle):
        # Data
        ally_team = self.get_pokemon_team_data(battle.team,battle)
        enemy_team = self.get_pokemon_team_data(battle.opponent_team,battle)
        fixed_input_data = np.concatenate([ally_team, enemy_team])
        return fixed_input_data

    def get_pokemon_team_data(self,team,battle):
        pokemon_array = -np.ones(6)
        ...
        for i,pokemon in enumerate(team.values()):
            ...
            moves_healing = -np.ones(4)
            moves_hits = -np.ones(4)
            status = 0
